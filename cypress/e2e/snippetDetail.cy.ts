describe('Add snippet tests', () => {
  beforeEach(() => {
    ...
    cy.intercept('GET', Cypress.env('VITE_MANAGER_URL')+"/manager/snippets/*").as("getSnippetById")
    cy.intercept('GET', Cypress.env('VITE_MANAGER_URL')+"/manager/snippets*").as("getSnippets")

    cy.visit("/")

    cy.wait("@getSnippets")
     // TODO comment this line and uncomment 19 to wait for the real data
    cy.get('.MuiTableBody-root > :nth-child(1) > :nth-child(1)').click();
  })

  it('Can share a snippet ', () => {
    cy.intercept('POST', Cypress.env('VITE_MANAGER_URL')+"/manager/share").as("shareSnippet")
  ...
  it('Can format snippets', function() {
    cy.intercept('POST', Cypress.env('VITE_MANAGER_URL')+"/run/format").as("formatSnippet")
  ...
  it('Can save snippets', function() {
    cy.intercept('PUT', Cypress.env('VITE_MANAGER_URL')+"/manager/snippets/*").as("updateSnippet")
  ...
  it('Can delete snippets', function() {
    cy.intercept('DELETE', Cypress.env('VITE_MANAGER_URL')+"/manager/*").as("deleteSnippet")
